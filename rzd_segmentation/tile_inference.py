...
import albumentations.pytorch as pytorch_albu


torch_aug = pytorch_albu.ToTensorV2()

def inference_on_image(model, image, num_classes, tile_size=(512, 512), tile_step=(384, 384)):
    tiler = ImageSlicer(image.shape, tile_size=tile_size, tile_step=tile_step, weight= "pyramid")
    tiles = [torch_aug(image=tile)["image"] for tile in tiler.split(image)]
    merger = TileMerger(tiler.target_shape, num_classes, tiler.weight)
    
    with torch.no_grad():
        for tiles_batch, coords_batch in DataLoader(list(zip(tiles, tiler.crops)), batch_size=8):
            tiles_batch = tiles_batch.cuda()
            pred_batch = model(tiles_batch)
            merger.integrate_batch(pred_batch, coords_batch)
    
    output_image = np.moveaxis(merger.merge().data.cpu().numpy(), 0, -1)
    output_image = tiler.crop_to_orignal_size(output_image).squeeze()
